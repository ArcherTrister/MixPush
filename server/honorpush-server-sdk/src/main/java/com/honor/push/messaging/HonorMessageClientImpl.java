...
import com.honor.push.reponse.TopicListResponse;
import com.honor.push.reponse.TopicSendResponse;
...
public class HonorMessageClientImpl implements HonorMessageClient {
    ...
    private HonorMessageClientImpl(Builder builder) {
        this.HcmPushUrl = MessageFormat.format(PUSH_URL + "/api/v1/{0}/sendMessage", builder.appId);
    ...
    @Override
    public SendResponse send(Message message, boolean validateOnly, String accessToken) throws HonorMesssagingException {
        try {
            return sendRequest(message, validateOnly, accessToken);
        } catch (IOException e) {
            throw new HonorMesssagingException(HonorMessaging.INTERNAL_ERROR, "Error while calling HCM backend service", e);
        }
    }

    @Override
    public SendResponse send(TopicMessage message, String operation, String accessToken) throws HonorMesssagingException {
        try {
    ...
    private SendResponse sendRequest(TopicMessage message, String operation, String accessToken) throws IOException, HonorMesssagingException {
        ...
        if (statusCode == 200) {
            ...
            if (StringUtils.equals(code, "80000000")) {
                ...
                throw new HonorMesssagingException(HonorMessaging.KNOWN_ERROR, errorMsg);
            }
        }
     ...
     */
    private SendResponse sendRequest(Message message, String accessToken) throws IOException, HonorMesssagingException {
...
//        String aqa = JSON.toJSONString(map);
    ...
    private HonorMesssagingException createExceptionFromResponse(HttpResponseException e) {
        String msg = MessageFormat.format("Unexpected HTTP response with status : {0}, body : {1}", e.getStatusCode(), e.getMessage());
        return new HonorMesssagingException(HonorMessaging.UNKNOWN_ERROR, msg, e);
    }

    static HonorMessageClientImpl fromApp(HonorApp app) {
        String appId = ImplHonorTrampolines.getAppId(app);
        return HonorMessageClientImpl.builder()
                .setAppId(appId)
    ...
    static final class Builder {
        ...
        public HonorMessageClientImpl build() {
            return new HonorMessageClientImpl(this);
        }
    }
}
